...
def betterStockSimulation(data, capital):
    stock = 0
    capital_over_time = []
    days = []
    capital_over_time.append(0)
    ...
    starting_capital = capital
    for i in range(1, 1000):
        if data.MACD[i - 1] < data.Signal[i - 1] and data.MACD[i] >= data.Signal[i] and data.Price[i] < data.TrendLine[i]:
            capital, stock = buy(data, capital, stock, i)
        if data.MACD[i - 1] > data.Signal[i - 1] and data.MACD[i] <= data.Signal[i] and data.Price[i] > data.TrendLine[i]:
            capital, stock = sell(data, capital, stock, i)
        capital_over_time.append(
            calculateCapital(data, capital, stock, i) - calculateCapital(data, capital, stock, i - 1))
        days.append(i)
    sell(data, capital, stock, 999)
    print('Starting capital:' + str(round(starting_capital, 2)))
    print('Closing capital: ' + str(round(capital, 2)))
